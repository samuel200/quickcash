...
function gameToBoard(cells){
  let board = ""
  for (var i = 0; i < 9; i++) {
    for (var j = 0; j < 9; j++) {
      var cell = cells[i][j];
      board += cell.value ? cell.value : "0";
    }
  }
  return board;
}

function boardToGame(board, time) {
  var array = [];
  for (var i = 0; i < 81; i++) {
    if (board[i] === '0') {
      array.push(null);
  ...
  return newGame(game, time);
}

function populateBoard(cells, newBoard){
  const myCells = cells;
  let count = 0;

  for (var i = 0; i < 9; i++) {
    for (var j = 0; j < 9; j++) {
      const newVal = newBoard[count];
      if(myCells[i][j].editable && newVal !== "0"){
        myCells[i][j].value = newVal;
      }
      count++;
    }
  }
